...
**Eval 3: MCP Integration (Applied Skill)**
...
**Eval 4: Extension Lifecycle (Development Awareness)**
...
**Eval 5: IDE Integration (Workflow Improvement)**
...
**Gap Identified**:
Missing **5 critical features** released after initial writing:
...
- Students miss major productivity features (IDE integration)
- Students can't contribute to ecosystem (no extension development knowledge)
- Students lack security awareness (tool filtering)

**User Pain**:
"I followed the chapter but Gemini CLI has features I didn't learn about. Am I missing something?"
...
1. **Update Chapter 6 with Verified Intelligence**
   ...
   - Ensure 100% technical accuracy (no assumptions, no hallucinations)
   - Source citations for all configuration examples

2. **Integrate 5 Missing Critical Features**
   - CLI MCP management (new commands)
   - IDE integration (new workflow)
   - Extension lifecycle (development workflow)
   - OAuth authentication (secured servers)
   - Tool filtering (security)

3. **Maintain Constitutional Alignment**
   - Principle 7: Technical Accuracy and Currency (verified claims only)
   - Principle 13: Graduated Teaching (direct commands → AI companion → AI orchestration)
   - Core Philosophy #1: AI Development Spectrum (teach WHEN to use AI vs direct commands)

4. **Preserve Existing Pedagogical Quality**
   - Keep strong "Try With AI" prompts (Chapter 1 format)
   - Maintain cognitive load limits (max 7 concepts per section)
   - Preserve accessibility (Grade 7-8 reading level)

### Secondary Objectives

5. **Strategic AI Usage Throughout**
   - Clear distinction: direct commands (installation, CLI operations) vs AI collaboration (understanding concepts, troubleshooting)
   - Avoid over-engineering simple tasks with AI
   - Model spec-first thinking where appropriate
...
**Content Updates** (6 Lessons Total):
- ✅ Lesson 1: Update with latest Gemini CLI positioning (IDE integration mention, Qwen Code brief)
- ✅ Lesson 2: Update installation verification, add troubleshooting for common errors
- ✅ Lesson 3: Verify all built-in tools examples against verified docs
- ✅ Lesson 4: Update context window comparison (verify token counts)
- ✅ Lesson 5: **REVISED & EXPANDED** - MCP Servers and Integration (CLI commands, OAuth)
- ✅ Lesson 6: **NEW LESSON** - Extensions, Security & IDE Integration

**New Sections in Lesson 5** (MCP Focus):
1. Part 2: CLI MCP Management (NEW - 7 min)
   - `gemini mcp add` for stdio/HTTP/SSE servers
   - `gemini mcp list` for status checking
   - `gemini mcp remove` for cleanup
   - CLI vs manual JSON editing comparison

2. Part 3: OAuth for MCP Servers (NEW - 5 min)
   - `/mcp auth` command (high-level)
   - Automatic token management
   - Browser-based authentication flow
   - Use case: Secured APIs

**New Lesson 6** (Extensions & IDE Focus):
1. Part 1: Extension Development Workflow (NEW - 6 min)
...
2. Part 2: Tool Filtering for Security (NEW - 5 min)
   - `includeTools` (allowlist)
   - `excludeTools` (blocklist)
   - Real-world security scenario
   - Best practices

3. Part 3: IDE Integration (NEW - 6 min)
   - `/ide install` command
   - `/ide enable` connection
   - Benefits: 10 recent files, cursor position, native diffs
   - VS Code workflow demonstration

4. Part 4: Choosing the Right Workflow (NEW - 3 min)
   - Decision framework: MCP vs Extensions vs IDE
